...
AlterSetting(Oid databaseid, Oid roleid, VariableSetStmt *setstmt)
{
	...
	if (setstmt->kind == VAR_RESET_ALL)
	{
		if (HeapTupleIsValid(tuple))
		{
			...
			if (!isnull)
				new = GUCArrayReset(DatumGetArrayTypeP(datum), true);
	...
	else if (HeapTupleIsValid(tuple))
	{
		...
		if (valuestr)
			a = GUCArrayAdd(a, setstmt->name, valuestr, true);
		else
			a = GUCArrayDelete(a, setstmt->name, true);
	...
	else if (valuestr)
	{
		...
		a = GUCArrayAdd(NULL, setstmt->name, valuestr, true);
