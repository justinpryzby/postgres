...
AlterSetting(Oid databaseid, Oid roleid, VariableSetStmt *setstmt)
{
	...
	if (setstmt->kind == VAR_RESET_ALL)
	{
		if (HeapTupleIsValid(tuple))
		{
			...
			if (!isnull)
				new = GUCArrayReset(DatumGetArrayTypeP(datum), PGC_S_INTERACTIVE);
	...
	else if (HeapTupleIsValid(tuple))
	{
		...
		if (valuestr)
			a = GUCArrayAdd(a, setstmt->name, valuestr, PGC_S_INTERACTIVE);
		else
			a = GUCArrayDelete(a, setstmt->name, PGC_S_INTERACTIVE);
	...
	else if (valuestr)
	{
		...
		a = GUCArrayAdd(NULL, setstmt->name, valuestr, PGC_S_INTERACTIVE);
