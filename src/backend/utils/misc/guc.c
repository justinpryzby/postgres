	...
	bool restricted, bool is_session)
{
	...
	/* Do not issue warnings for non-persistent changes */
	if (source == PGC_S_SESSION || source == PGC_S_TEST)
		return true;
	...
	foreach(l, elemlist)
	{
		...
		if (is_session)
			ereport(WARNING,
					...
					errhint("New sessions will fail to start with new configuration."));
		else
			ereport(WARNING,
					errcode_for_file_access(),
					errmsg("could not access file \"%s\"", filename),
					// XXX: PGC_S_FILE
					errdetail("The server will fail to start with this setting."),
