...
-- Exercise recursion
-- Note that output can be unstable if subdirs are dynamically removed, so pgsql_tmp is excluded.
select path, filename, type from pg_ls_dir_metadata('.', true, false, true) where
path in ('base', 'global', 'global/pg_control', 'global/pg_filenode.map', 'PG_VERSION', 'pg_multixact', 'pg_multixact/members', 'pg_multixact/offsets', 'pg_wal', 'pg_wal/archive_status')
